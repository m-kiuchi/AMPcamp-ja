...
ソーシャルネットワークや言語モデリングなどの分野でグラフデータの重要性と規模が広がっています。
...
トレーニング前半を通して、以下のようなソーシャル友人関係を表すプロパティグラフを使っていきます。データとしてはとても単純ですが、グラフデータのモデルとGraphXを学ぶのに適しています。以下のデータにおける頂点にはユーザ名と年齢が含まれ、辺はベクトルとなります。

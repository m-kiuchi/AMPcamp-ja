...
もし、２行目のコマンドの後に何も出力がでなければ、rJavaが正常にインストールされた証拠です。
...
## 6-2. データセットの確保
[1-1. 用意するもの](introduction.md)を参照してください。
適したtraining-downloads.zipファイルをダウンロードして展開したら、準備完了です。
...
### <font color="black">6-4-(1). はじめに入力ファイルからdataという名のRDDを作成しましょう。</font>
...
### <font color="black">6-4-(2). データの中身を見てみましょう。</font>
RDDのtake操作を使って初めの2個のRDDを見ていきます。
...
### <font color="black">6-4-(3). データセット全体で記録がどれだけあるのかを調べましょう。</font>
...
この動作を行うにはブラウザを開いてhttp://sparkホスト:4040を入力してください。
...
### <font color="black">6-4-(4). [SparkSQL](spark-sql.md)の練習を思い出してみてください。</font>
このデータの構成も似ている(pageId, title, modifiedTime, text, username)ことがわかります。
それでは、このデータをパースしフィールドごとにRDDのリストを作成してみましょう。
...
上のコマンドをSparkシェルに入力した時はSparkがRDDを定義しますが、Spark特有の遅延評価のために計算処理は行われていないません。
次にparsedRDDに何らかのアクションが実行されたら、Sparkはメモリにデータセットをキャッシュします。

### <font color="black">6-4-(5). 幾つの記録がparsedRDDにあるかを数える。</font>
...
元データには行ごとに一つの記録があるはずですので、この数は前の数と同じはずです。

### <font color="black">6-4-(6). もう少し面白いことをしましょう。</font>
データセットを探索し、ユーザーが貢献した数のヒストグラムを作りましょう。
簡単に手順を説明すると以下のようになります。まず、ユーザーネームだけを切り取り名無しの記録を取り除きます。

ユーザーネームの切り取りと名無しの取り除く
...
そして、各キーの値を総合します。
Sparkでこのパターンの処理に便利なのがreduceByKeyという関数です。
reduceByKeyの二つ目の引数が何個のリデューサを利用するかを決めます。 デフォルトではリデュース関数は累積的かつ統合的で、マップ側にコンバイナを割り当てます。

この例では入力のRDDはパーティションを8個持っていますので、8個のリデューサを使いましょう。
...
上記の操作の結果できたUserCountsはローカルのリストで、様々なRコマンドを利用して操作することができるようになりました。
例としてTop10のユーザーを取得することはこのリストをソートして最後の10エレメントを取る方法で調べることができます。
...
### <font color="black">6-4-(7). 応用的な実践として、上のコードを使って以下の課題を解いてみてください。</font>
...
### <font color="black">6-4-(8). RDD APIで使用できる全ての操作はhelp(package=SparkR)で参照できます。</font>

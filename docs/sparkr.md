...
### 6-4-(1). はじめに入力ファイルからdataという名のRDDを作成しましょう。
...
### 6-4-(3). データセット全体で記録がどれだけあるのかを調べましょう。
...
### 6-4-(4). [SparkSQL](spark-sql.md)の練習を思い出してみてください。
...
### 6-4-(7). 応用的な実践として、上のコードを使って以下の課題を解いてみてください。
...
### 6-4-(8). RDD APIで使用できる全ての操作はhelp(package=SparkR)で参照できます。
